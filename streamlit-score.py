...
import pandas as pd
from score_boosting import disease_product_mapping, add_score, exclude_allergies, exclude_pregnancy, score_life_stage, score_disease_tags, score_products
...
health_conditions = sorted([
	...
	"autoimmune diseases", "brachycephalic_syndrome", "bladder_stones", "canine_parvovirus",
...
with st.form("pet_form"):
    ...
    if main_issue != "-- None --":
        other_options = [h for h in health_conditions if h != main_issue]
    else:
        other_options = health_conditions
...
if submitted:
    ...
    if errors:
        ...
        other_issues_list = [issue.strip().lower() for issue in other_issues]
        ...
        product_ids, scores = score_products(pet_info, df_products)
        scored_df = pd.DataFrame({'Product_id': product_ids, 'Score': scores})
        top_recommendations = scored_df.sort_values(by='Score', ascending=False).head(10)
        

        st.subheader("Top Food Recommendations")
        st.dataframe(top_recommendations[["Product_id", "Score"]])
        
        recommended_products = top_recommendations.merge(df_products[['Product_id', 'Product_Name']], on='Product_id', how='left')
